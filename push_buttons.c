...
static My_float_pit_t g_signals_freq;
...
{
	uint32_t input_value = GPIO_read_port(GPIO_C);
	input_value &= 0xC40;

	if(B0 == input_value)
	{
		change_period(DELAY_FOR_15HZ);
	}
	else if(B1 == input_value)
	{
		change_period(DELAY_FOR_10HZ);
	}
	else if(BOTH_PB == input_value)
	{
		change_period(DELAY_FOR_5HZ);
	}
	else if((SW2 == input_value) && (GPIO_read_pin(GPIO_A, bit_4)))
	{
		start_sawtooth();
	}
}
...
void buttons_pressed(void) /**Default signal when system runs for the first time*/
{
	...
	{
		PIT_delay(PIT_3, SYSTEM_CLOCK, DELAY_FOR_SW);
		start_sin();
	}
	else
	{
